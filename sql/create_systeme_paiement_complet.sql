-- ============================================
-- SYSTÈME DE PAIEMENT ET CONTRACTUALISATION
-- KaziPro - Installation Complète
-- ============================================

-- PARTIE 1: TABLES DE CONFIGURATION
-- ============================================
...
-- Index
CREATE INDEX IF NOT EXISTS idx_config_paiement_modified ON configuration_paiement_globale(modified_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_historique_config_admin ON historique_config_paiement(admin_id);
CREATE INDEX IF NOT EXISTS idx_historique_config_date ON historique_config_paiement(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_config_prestataire_id ON configuration_paiement_prestataire(prestataire_id);
CREATE INDEX IF NOT EXISTS idx_config_prestataire_actif ON configuration_paiement_prestataire(paiement_via_kazipro);

-- PARTIE 2: FRAIS ET TEMPLATES
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_frais_deplacement_prestataire ON frais_deplacement_config(prestataire_id);
CREATE INDEX IF NOT EXISTS idx_frais_deplacement_actif ON frais_deplacement_config(actif);
...
CREATE INDEX IF NOT EXISTS idx_templates_prestataire ON conditions_paiement_templates(prestataire_id);
CREATE INDEX IF NOT EXISTS idx_templates_defaut ON conditions_paiement_templates(est_defaut);

-- PARTIE 3: CONTRATS, PAIEMENTS, LITIGES
-- ============================================
...
CREATE TABLE IF NOT EXISTS paiements (
  ...
  methode_paiement TEXT 
    CHECK (methode_paiement IN ('mpesa', 'airtel_money', 'orange_money', 'carte_bancaire', 'especes', 'virement', 'direct')),
...
CREATE TABLE IF NOT EXISTS litiges (
  ...
  categorie TEXT CHECK (categorie IN ('qualite_travaux', 'delai', 'paiement', 'materiel', 'autre')),
...
-- PARTIE 4: MODIFIER TABLES EXISTANTES
-- ============================================
...
BEGIN
  ...
  RAISE NOTICE '- configuration_paiement_globale';
  RAISE NOTICE '- historique_config_paiement';
  RAISE NOTICE '- configuration_paiement_prestataire';
  RAISE NOTICE '- frais_deplacement_config';
  RAISE NOTICE '- conditions_paiement_templates';
  RAISE NOTICE '- contrats';
  RAISE NOTICE '- paiements';
  RAISE NOTICE '- litiges';
  RAISE NOTICE '- devis_pro (modifié)';
  RAISE NOTICE '- missions (modifié)';
END $;
