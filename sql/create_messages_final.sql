...
-- This script properly handles existing conversations table

-- Step 1: Drop the existing conversations table (not view)
...
-- Step 3: Create view for conversations (list of unique conversations)
-- This view shows all unique conversations for the current user
...
SELECT 
  ...
  (
    ...
    WHERE m.receiver_id = auth.uid() 
      AND m.read = FALSE
      AND (
        (m.sender_id = lm.other_user_id AND m.receiver_id = auth.uid())
        OR (m.sender_id = auth.uid() AND m.receiver_id = lm.other_user_id)
      )
  ) as unread_count
...
BEGIN
  ...
  RAISE NOTICE '✅ Conversations view created';
  RAISE NOTICE '';
  RAISE NOTICE '📊 You can now:';
  RAISE NOTICE '   - Send and receive messages';
  RAISE NOTICE '   - View conversations list';
  RAISE NOTICE '   - Mark messages as read';
  RAISE NOTICE '   - Count unread messages';
END $$;
